...
impl Store {
    ...
    pub fn save_to_bmff_fragmented<P: AsRef<Path>>(
        ...
        // write the signed manifest to each output init segment (or XMP-only for remote modes)
        for (init_path, output_file) in init_paths.iter().zip(output_files.iter()) {
            if embed_jumbf {
                save_jumbf_to_file(&final_jumbf, output_file, Some(output_file))?;
            } else {
                std::fs::copy(init_path, output_file).map_err(|e| {
                    Error::BadParam(format!("could not restore init segment: {e}"))
                })?;
                if let Some(ref url) = remote_url {
                    let io_handler = crate::jumbf_io::get_assetio_handler_from_path(output_file)
                        .ok_or(Error::UnsupportedType)?;
                    let embed_handler = io_handler
                        .remote_ref_writer_ref()
                        .ok_or(Error::XmpNotSupported)?;
                    embed_handler
                        .embed_reference(
                            output_file,
                            RemoteRefEmbedType::Xmp(url.to_string()),
                        )
                        .map_err(|_| Error::EmbeddingError)?;
                }
            }
    ...
    pub fn load_from_file_and_fragments(
        ...
        let (manifest_bytes, remote_url) =
            Store::load_jumbf_from_stream(asset_type, init_segment, context)?;
